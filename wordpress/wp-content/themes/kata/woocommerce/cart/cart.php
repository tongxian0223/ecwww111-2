				...
				foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
					...
					if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
						...
						<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
							...
							<td class="product-name kata-cart-product" data-title="<?php esc_attr_e( 'Item', 'kata' ); ?>">
							<?php
							...
							if ( ! $product_permalink ) {
								...
								echo wp_kses_post( apply_filters( 'woocommerce_cart_item_name', sprintf( '<a class="product-name" href="%s">%s</a>', esc_url( $product_permalink ), $_product->get_title() ), $cart_item, $cart_item_key ) );
							}
					...
					<td colspan="6" class="actions">
						...
						<?php if ( wc_coupons_enabled() ) { ?>
							<div class="coupon">
								...
								<label for="coupon_code"><?php esc_html_e( 'Coupon:', 'kata' ); ?></label> <input type="text" name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="<?php esc_attr_e( 'Coupon code', 'kata' ); ?>" /> <button type="submit" class="button apply-coupon" name="apply_coupon" value="<?php esc_attr_e( 'Apply', 'kata' ); ?>"><?php esc_html_e( 'Apply', 'kata' ); ?></button>
